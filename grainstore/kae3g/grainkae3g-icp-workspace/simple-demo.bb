...
(println "🌾 GrainThrift Demo: Simple Base Case")

(defn create-simple-demo
  [canister-name]
  (println "📁 Creating simple demo for:" canister-name)
  
  ;; Create output directory
  (fs/create-dirs (str "src/" canister-name "_motoko"))
  
  ;; Write simple Motoko code for demo
...
<body>
  ...
  (println "✅ Simple demo created!")
  (println "📁 Output: src/" canister-name "_motoko/main.mo"))

(defn deploy-demo
  [canister-name]
  (println "🚀 Deploying demo canister:" canister-name)
  (shell "dfx deploy" canister-name)
  (println "✅ Demo deployed!"))

(defn test-demo
  [canister-name]
  (println "🧪 Testing demo canister:" canister-name)
  (println "Status:" (:out (shell "dfx canister call" canister-name "status")))
  (println "Product:" (:out (shell "dfx canister call" canister-name "getProduct" "\"rl-polo-classic\""))))

(defn -main
  [& args]
  (let [command (first args)
        canister-name (or (second args) "grainthrift-demo")]
    
    (case command
      "create"
      (create-simple-demo canister-name)
      
      "deploy"
      (deploy-demo canister-name)
      
      "test"
      (test-demo canister-name)
      ...
      (do
        ...
        (println "")
        (println "Usage:")
        (println "  bb simple-demo.bb create [canister-name]")
        (println "  bb simple-demo.bb deploy [canister-name]")
        (println "  bb simple-demo.bb test [canister-name]")
        (println "  bb simple-demo.bb demo [canister-name]")
        (println "  bb simple-demo.bb help")
        (println "")
        (println "Examples:")
        (println "  bb simple-demo.bb demo")
        (println "  bb simple-demo.bb demo grainthrift-simple"))
      
      (do
        (println "❌ Unknown command:" command)
        (println "Run 'bb simple-demo.bb help' for usage information.")))))
