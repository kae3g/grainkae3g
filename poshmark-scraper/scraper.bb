...
(defn search-poshmark-cotton
  ...
  ;; Simulated Poshmark search results
...
(defn scrape-and-update-grainthrift
  ...
  (let [poshmark-items (search-poshmark-cotton query)
    ...
    ;; Save Motoko code
    (spit "output/products.mo" motoko-code)
    
    ;; Save HTML code
    (spit "output/products.html" html-code)
    
    ;; Save JSON data
    (spit "output/products.json" (str/join "\n" (map pr-str grainthrift-products)))
    
    (println "✅ Scraped" (count grainthrift-products) "products")
    (println "📁 Motoko code: output/products.mo")
    (println "📁 HTML code: output/products.html")
    (println "📁 JSON data: output/products.json")
